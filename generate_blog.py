...
# --- FIX: Robust Content Parsing to find Title and Summary ---
raw_lines = content_raw.split('\n')
generated_title_raw = title # Fallback
summary = "Summary could not be extracted."
start_index = 0

# 1. Find the Title (first non-empty line)
for i, line in enumerate(raw_lines):
    if line.strip():
        generated_title_raw = line.strip()
        start_index = i + 1
        break

# 2. Find the Summary (second non-empty line)
if start_index > 0:
...
# 3. The remaining content is the body
body_content = '\n'.join(raw_lines[start_index:]).strip()
# --- END FIX: Robust Content Parsing ---
...
# The filename now includes the 'posts/' prefix for correct linking in the JSON
filename = f"posts/{slug}_{date_str}.html"
...
post_data = {
    "title": generated_title_raw, # Use the model-generated title
    "excerpt": summary,
    "link": filename, # Stores the full path including 'posts/'
    "date": date_str
}
